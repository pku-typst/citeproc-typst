...
// Handle CSL path - use if-else only for file reading, not for document structure
#let csl-path = sys.inputs.at("csl", default: none)
#let bib-content = read("/tests/test-gb7714.bib")

// Read CSL or use a minimal fallback (will show error in output)
#let csl-content = if csl-path != none {
  read("/" + csl-path)
} else {
...
#align(center)[
  ...
  #v(0.3em)
  #text(size: 9pt, fill: if csl-path == none { red } else {
    gray
  })[#display-path]
]
